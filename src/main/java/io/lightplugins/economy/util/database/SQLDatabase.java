...
import io.lightplugins.economy.util.database.model.DatabaseTypes;
...
public abstract class SQLDatabase {
    ...
    protected final LightEconomy plugin;

    SQLDatabase(LightEconomy plugin) {
        this.plugin = plugin;
    }
    ...
    public CompletableFuture<ResultSet> executeQueryAsync(String sql, Object... replacements) {
        ...
        new BukkitRunnable() {
            @Override
            public void run() {
                try (Connection c = getConnection(); PreparedStatement statement = prepareStatement(c, sql, replacements); ResultSet set = statement.executeQuery()) {
                    while (set.next()) {
                        count.getAndIncrement();
                    }
                    future.complete(set);
                    Bukkit.getLogger().log(Level.INFO, "Query executed: " + sql + " (Replacement values: " + Arrays.toString(replacements) + "). Found " + count.get() + " results.");
                } catch (SQLException e) {
                    e.printStackTrace();
        ...
        }.runTaskAsynchronously(LightEconomy.instance);
